	...
	window.JCCatalogElement.prototype = {
		...
		buttonShow: function () {
			...
			$.ajax({
				...
				success: function (data) {
					if(data.STATUS == 'OK') {
						$('.href_button').css('display','block');
						$('.buy_button').css('display','none');
					} else {
						$('.buy_button').css('display','block');
						$('.href_button').css('display','none');
					}
				}
			});
		...
		drawImages: function(images)
		{
			...
			for (i = 0; i < xImages.length; i++)
			{
				...
				if(xImages.length > 1) {
					var img = $('<li style="width: 100px; float: left; display: block;">\n' +
						'<a href="' + xImages[i].SRC + '" class="cloud-zoom-gallery" rel="useZoom: \'zoom1\', smallImage: \'' + x2Images[i].SRC + '\' ">\n' +
						'<img src="' + xImages[i].SRC + '" alt=" ' + this.config.alt + ' " style="width:80px">\n' +
						'</a>\n' +
						'</li>');
				} else {
					var img = $('<li>\n' +
		...
		basketResult: function(arResult)
		{
			...
			else
			{
				...
				if (arResult.STATUS === 'OK')
				{
					...
					$('.product-item-detail-info-container').append(buttonInCart);
