...
{ ?>
 <li>
            <a href="<?=$arResult['sUrlPathParams']; ?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]+1)?>&SIZEN_<?=$arResult["NavNum"]?>=<?=$arResult['NavPageSize']; ?>"><i
    ...
    while($NavRecordGroup <= $arResult["NavPageCount"])
    {
        ...
        if ($NavRecordGroup == $arResult["NavPageNomer"])
        {
            ?><li class="active" title="<?=GetMessage('nav_page_current_title'); ?>">
            <a class="active" href="">  <?=$NavRecordGroup; ?> </a>
             </li>
       <? }
        elseif ($NavRecordGroup == 1 && $arResult["bSavePage"] == false)
        {
