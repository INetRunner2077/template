            ...
            if(0 < $intProductIBlockID){
                if($_REQUEST["add_props"]=="Y"){
                    ...
                    if ($intProductIBlockID == $_REQUEST["iblockID"]){
                        ...
                        $prop_code = 'ARTICLE';
                        $prop_parent_code = 'CML2_ARTICLE';
                        $need_fill = true;

                        if(in_array($prop_code, $arSkuProp)) {
                        ...
                        if($need_fill) {
                            $arElement_ = CIBlockElement::GetList(array(), array('ID' => $_REQUEST["item"]), false, false, array('PROPERTY_'.$prop_code, 'PROPERTY_CML2_LINK'))->Fetch();
                            if(!$arElement_['PROPERTY_'.$prop_code.'VALUE']) {
                                $parent_prop = CIBlockElement::GetProperty(
                                    $_REQUEST["iblockID"],
                                    $arElement_["PROPERTY_CML2_LINK_VALUE"],
                                    array("sort"=>"asc", "enum_sort" => "asc", "value_id"=>"asc"),
                                    array('CODE' => $prop_parent_code)
                                )->Fetch();
                                $product_properties[] = array(
                                    'NAME' => $parent_prop['NAME'],
                                    'CODE' => $parent_prop['CODE'],
    ...
    if($_REQUEST["subscribe_item"] == "Y"){
        if(class_exists('\Bitrix\Catalog\Product\SubscribeManager')){
            ...
            if($itemID && ($bSubscribeProducts || $userId))
            {
                ...
                $arSubscribeList = CMshop::getUserSubscribeList($userId);
